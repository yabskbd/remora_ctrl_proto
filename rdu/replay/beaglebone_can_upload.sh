...
find /var/log/candump -type f -name 'candump*.gz' -printf '%f\n' | xargs -I % sh -c "DEST_FILE_NAME=\$(echo % | sed -E \"s/candump(.*).log.gz/candump\1_$HOST_ID.log.gz/g\"); rclone moveto /var/log/candump/% digitalocean:testingcandump/dumps/\$DEST_FILE_NAME"
